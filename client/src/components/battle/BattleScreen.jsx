      ...
      <div style={{flex:1,display:'grid',gridTemplateColumns:'1fr 2fr',overflow:'hidden',minHeight:0}}>
        ...
        <div style={{display:'flex',flexDirection:'column',overflow:'hidden'}}>
          {is_active && !i_am_defeated && selected_char_id==='MC' ? (
            <MCDetailPanel is_attacking={is_attacking} onAttack={doAttack} />
...
// ═══════════════════════════════════════════════════════════════════════
// MC CARD (overview) — slot 0
// ═══════════════════════════════════════════════════════════════════════
function MCCard({ cardRef, is_selected, onSelect }) {
  return (
    <div ref={cardRef} onClick={onSelect} style={{
      display:'flex',gap:8,alignItems:'center',padding:'6px 8px',borderRadius:8,
      background:is_selected?'rgba(240,192,48,0.15)':'rgba(255,255,255,0.03)',
      ...
      <div style={{flex:1}}>
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:3}}>
          <span style={{fontSize:'0.7rem',fontFamily:'var(--font-display)',fontWeight:700,color:'var(--text-gold)'}}>Sky-Wanderer</span>
          <span style={{fontSize:'0.55rem',fontFamily:'var(--font-mono)',color:'var(--text-dim)'}}>MC</span>
        </div>
        <div style={{fontSize:'0.6rem',color:'var(--text-dim)',fontFamily:'var(--font-mono)'}}>Fighter · Lv 1 · Slot 0</div>
      </div>
    </div>
  );
}
...
// MC DETAIL PANEL
// ═══════════════════════════════════════════════════════════════════════
function MCDetailPanel({ is_attacking, onAttack }) {
  return (
    <div style={{flex:1,display:'flex',flexDirection:'column',overflow:'hidden',background:'var(--bg-deep)'}}>
      <div style={{padding:'14px 16px',borderBottom:'1px solid var(--border-dim)',
        ...
        <div style={{display:'flex',gap:12,alignItems:'center'}}>
          ...
          <div>
            <div style={{fontFamily:'var(--font-display)',fontSize:'1rem',fontWeight:700,color:'var(--text-gold)',marginBottom:4}}>
              Sky-Wanderer
            </div>
            <div style={{fontSize:'0.7rem',color:'var(--text-dim)',fontFamily:'var(--font-mono)'}}>Fighter · Lv 1 · Main Character</div>
            <div style={{fontSize:'0.65rem',color:'var(--text-dim)',marginTop:6,lineHeight:1.5}}>
              The MC leads the party. Skills and class system coming in a future update.
            </div>
          </div>
        </div>
      </div>
      <div style={{flex:1,display:'flex',alignItems:'center',justifyContent:'center',color:'var(--text-dim)',fontSize:'0.8rem',fontFamily:'var(--font-mono)'}}>
        MC class system — v2.0
      </div>
          ...
          <div style={{flex:1}}>
            ...
            <div style={{marginBottom:4}}>
              <div style={{display:'flex',justifyContent:'space-between',marginBottom:2}}>
                ...
                <span style={{fontSize:'0.62rem',color:'#60cc60',fontFamily:'var(--font-mono)',fontWeight:700}}>{formatNumber(char.hp)} / {formatNumber(char.hp_max)}</span>
              </div>
            ...
            <div>
              ...
              {char.charge_bar>=100&&<div style={{fontSize:'0.55rem',color:'#f0c030',fontFamily:'var(--font-mono)',fontWeight:700,marginTop:2}}>★ READY — fires on Attack</div>}
            </div>
          </div>
        </div>
      ...
      <div style={{flex:1,overflowY:'auto',padding:'10px 14px',display:'flex',flexDirection:'column',gap:8}}>
        ...
        <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:8}}>
          {abilities.map(ab=>{
            ...
            const badge_clr = ab.effect_type==='DAMAGE'?'#ff6040':ab.effect_type==='BUFF'?'#4090ff':ab.effect_type==='HEAL'?'#38e060':ab.effect_type==='DEBUFF'?'#c060ff':'#888';
            return (
              <div key={ab.id} style={{borderRadius:10,overflow:'hidden',
                border:`1px solid ${ready?'rgba(80,160,80,0.4)':'var(--border-dim)'}`,
                background:ready?'rgba(20,50,20,0.4)':'rgba(255,255,255,0.02)',
                opacity:ready?1:0.5,transition:'all 0.15s',display:'flex',flexDirection:'column'}}>
                {/* Icon area */}
                <div style={{height:52,display:'flex',alignItems:'center',justifyContent:'center',
                  background:ready?'rgba(40,100,40,0.25)':'rgba(80,20,20,0.2)',
                  borderBottom:`1px solid ${ready?'rgba(80,160,80,0.3)':'var(--border-dim)'}`,position:'relative'}}>
                  <div style={{width:36,height:36,borderRadius:8,
                    background:`rgba(${ab.effect_type==='DAMAGE'?'255,80,32':ab.effect_type==='BUFF'?'64,144,255':ab.effect_type==='HEAL'?'56,224,96':'192,96,255'},0.22)`,
                    border:`2px solid ${ready?badge_clr:'rgba(160,40,40,0.5)'}`,
                    display:'flex',alignItems:'center',justifyContent:'center',
                    boxShadow:ready?`0 0 10px ${badge_clr}44`:'none',transition:'all 0.2s'}}>
                    {ready
                      ? <span style={{color:badge_clr,fontSize:'1.1rem'}}>{ab.effect_type==='DAMAGE'?'⚔':ab.effect_type==='BUFF'?'↑':ab.effect_type==='HEAL'?'♥':ab.effect_type==='DEBUFF'?'↓':'◆'}</span>
                      : <span style={{fontSize:'0.8rem',fontWeight:700,color:'var(--hp-red)'}}>{cd}t</span>
                    }
                  </div>
                  {!ready&&(
                    <div style={{position:'absolute',inset:0,display:'flex',alignItems:'center',justifyContent:'center',background:'rgba(0,0,0,0.45)',borderRadius:9}}>
                      <span style={{fontFamily:'var(--font-mono)',fontSize:'1rem',fontWeight:700,color:'var(--hp-red)'}}>{cd}</span>
                    </div>
                  )}
                  <div style={{position:'absolute',top:4,right:4,fontSize:'0.42rem',fontFamily:'var(--font-mono)',fontWeight:700,
                    color:badge_clr,background:'rgba(0,0,0,0.65)',padding:'1px 4px',borderRadius:3}}>
                    {ab.effect_type}
                  </div>
                </div>
                {/* Info */}
                <div style={{padding:'8px 9px',flex:1,display:'flex',flexDirection:'column',gap:3}}>
                  <div style={{fontFamily:'var(--font-mono)',fontSize:'0.7rem',fontWeight:700,color:ready?'var(--text-bright)':'var(--text-dim)'}}>{ab.name}</div>
                  ...
                  <div style={{display:'flex',gap:6,fontSize:'0.58rem',fontFamily:'var(--font-mono)',color:'var(--text-dim)',
                    ...
                    <span style={{color:ready?'rgba(80,200,80,0.7)':'var(--hp-red)'}}>{ready?'READY':`${cd}t left`}</span>
                  </div>
                  <button className={ready?'btn btn-primary':'btn btn-ghost'}
                    style={{width:'100%',padding:'5px',fontSize:'0.65rem',marginTop:4}}
                    disabled={!ready} onClick={()=>ready&&onUseAbility(ab.id,ab.name)}>
                    {ready?'▶ Use':'⏳ Cooldown'}
                  </button>
                </div>
              </div>
        ...
        {cat_char?.charge_attack&&(
          <div style={{padding:'10px 12px',borderRadius:10,background:'rgba(240,192,48,0.06)',border:'1px solid rgba(240,192,48,0.2)'}}>
            <div style={{fontSize:'0.55rem',color:'var(--text-dim)',fontFamily:'var(--font-mono)',letterSpacing:'0.06em',marginBottom:3}}>CHARGE ATTACK</div>
            <div style={{fontFamily:'var(--font-display)',fontSize:'0.85rem',fontWeight:700,color:'var(--text-gold)',marginBottom:3}}>{cat_char.charge_attack.name}</div>
