...
// ═══════════════════════════════════════════════════════════════════════
function MCDetailPanel({ mc_char, mc_class, grid_stats, is_attacking, onAttack, onUseAbility }) {
  const dead     = mc_char ? mc_char.hp <= 0 : false;
  const hp_pct   = mc_char ? mc_char.hp / mc_char.hp_max : 1;
  const hp_cls   = hp_pct > 0.6 ? '' : hp_pct > 0.3 ? ' yellow' : ' red';
  const ca_pct   = mc_char?.charge_bar || 0;
  const abilities = mc_char?.abilities || [];
  const eff_atk = mc_char?.base_atk && grid_stats
    ? Math.round((mc_char.base_atk + (grid_stats.grid_atk || 0)) * (grid_stats.normal_mult || 1) * (grid_stats.omega_mult || 1) * (grid_stats.ex_mult || 1))
    : null;
